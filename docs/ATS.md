...
companies already sit on ATS systems aplyx has shipped adapters for —
they were simply never added to any tenant list.
...
Oracle Recruiting Cloud, and Eightfold have no equivalent registry** —
every tenant added so far (Oracle's own, American Express, JPMorgan
...
2. **A dedicated research pass across large employers** (banks,
   ...
   public careers page, grep for known ATS hostnames) is cheap and
   mostly static-HTML-friendly; JS-rendered career sites (Morgan
   Stanley, PayPal, Deloitte, Walmart all returned no hits this pass)
   need the existing Playwright-network-inspection technique instead.
...
specifically because Google requires it for Google for Jobs indexing —
this is *why* Google's own job search can aggregate postings from nearly
...
Operator-directed: add The Muse (from the 2026-08-09 research pass —
free, public, no-login, native `level=Internship` filtering, confirmed
...
(the one pre-existing `harness:claude` failure is unrelated —
transcript-golden-key matching, not touched by this change); core/
...
being fetched, no carve-out), there's no single document to check here
— each customer's own site terms would govern their specific job board,
...
confirmed real via UCLA's own HR department pages, not a guessed slug)
— confirmed live: **zero server-rendered job data of any kind**, not
