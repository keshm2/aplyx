...
> during ats-account-credentials-plan.md Package 4/testing work
> (2026-08-23) — no code has been written against this. Follow the
...
1. **`docs/sms-notifications-plan.md`** (planning-only, not built) is
   ...
   - It explicitly rejected a **central, aplyx-owned relay** for SMS
     ...
     **This plan deliberately diverges from that for Discord — see
     ...
     is inherently multi-tenant — that's the platform's whole model).
...
- `field_clarification_requests` needs the same ownership-scoped RLS
  discipline as every other table in `docs/ats-account-credentials-plan.md`
  — a user must never be able to read or answer another user's
...
1. **Data model + RLS**: `discord_connections`,
   ...
   same rollback-tested-migration discipline as
   `docs/ats-account-credentials-plan.md`'s packages.
