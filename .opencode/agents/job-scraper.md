...
max_turns: 80
---
<!-- GENERATED from agents/bodies/job-scraper.md + agents/frontmatter/opencode/job-scraper.yaml — edit those sources and run scripts/validate/generate_agent_definitions.py -->
...
3d. For Eightfold-powered careers sites (a white-labeled ATS many large
   ...
   Playwright:
   `python3 scripts/jobs/fetch_eightfold_listings.py --search "<query>" --limit 200`
   It reads config/targets.json "eightfold_tenants" ("<host>/<domain>"
   ...
   - Eightfold listings carry NO JD text in list mode, but (unlike an
     ...
     candidate:
     `python3 scripts/jobs/fetch_eightfold_listings.py --jd-url '<posting-url>'`
     then re-canonicalize and upsert with the fetched jd_text. Never
