...
**Fields collected and stored locally** (`src/config/targets.json`'s `safe_fields`, gitignored): `first_name`, `last_name`, `preferred_name`, `email`, `phone`, `address_line1`, `address_line2`, `zip_code`, `location`, `linkedin_username`/`linkedin_url`, `github_username`/`github_url`, `graduation_date`, `gpa`, `authorized_to_work`, `require_sponsorship`, `citizenship_status`, `currently_enrolled`, `ethnicity`, `hispanic_or_latino`, `date_of_birth`, `gender`, `veteran_status`, `disability_status`. This includes protected-class demographic data (ethnicity, gender, date of birth, veteran status, disability status) alongside standard contact/identity PII. `gender`, `ethnicity`, `hispanic_or_latino`, and `date_of_birth` are optional free text; `veteran_status` and `disability_status` are required-to-answer (onboarding won't advance without a choice) but always include a "prefer not to answer" option among their fixed three, so this still never forces an actual disclosure. All six are excluded from the interest-letter LLM prompt context (`generate_interest_letter.py`'s `_EXCLUDED_PROFILE_KEYS`), and deliberately absent from the browser extension's autofill scope (`extension_bridge.py`'s `SAFE_FIELD_KEYS`, `ats.ts`'s `FieldKey`) — the Playwright-driven main apply agent is the only path that ever fills them, under the same exact-match/never-invent rule as every other `safe_fields` value.
