...
Steps mirror src/scripts/install/install.sh:
  ...
  3. Detect installed coding agents; write src/config/harness.json.
  ...
  8. Build the TUI (src/tui/) when node is available.
...
$projectRoot = $null
if ($scriptPath) {
  $maybeRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $scriptPath)))
...
if (-not $projectRoot) {
  ...
  # checkout someone points this script at directly ($projectRoot already
  # set, above). See $CruftPaths above for what and why.
  Remove-Cruft -Target $target
  Remove-Item $tgz -ErrorAction SilentlyContinue
  & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $target "src\scripts\install\install.ps1")
  exit $LASTEXITCODE
}
...
# inside it) never goes through the piped bootstrap above -
# $projectRoot is already set, so that block is skipped entirely. It
# needs the same trimming for the same reason: no .git directory is the
# reliable signal that this is an extracted archive, not a real clone a
# developer/contributor is working in (which must never have files
# silently deleted out from under it).
if (-not (Test-Path (Join-Path $projectRoot ".git"))) {
  Say "no .git directory found - trimming files a local install doesn't need to run."
  Remove-Cruft -Target $projectRoot
}
...
if (Test-Path "src\config\targets.json") {
  ...
  Copy-Item "src\config\targets.example.json" "src\config\targets.json"
  Say "created src/config/targets.json from the example - fill placeholders (or run 'aplyx setup')."
}

# --- 2b. Discord (optional, opt-in) ------------------------------------------
$discordLive = "src\config\discord_config.json"
...
if (Test-Path "src\config\harness.json") {
  Say "src/config/harness.json exists - keeping it."
} else {
  $harness = ""
  ...
  if ($harness) {
    [ordered]@{ harness = $harness } | ConvertTo-Json | Set-Content -Encoding UTF8 "src\config\harness.json"
    Say "wrote src/config/harness.json (harness: $harness)."
  } else {
...
# --- 6. Agent definitions ----------------------------------------------------
Invoke-Python @("src\scripts\validate\generate_agent_definitions.py")

# --- 7. Validate -------------------------------------------------------------
Invoke-Python @("src\scripts\validate\validate_local_config.py")
...
if ((Test-Path "src\tauri\package.json") -and (Get-Command npm -ErrorAction SilentlyContinue)) {
  ...
  if ($installApp -eq "y" -or $installApp -eq "Y") {
    ...
    $desktopFailed = $false
    try {
      & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $projectRoot "src\scripts\install\install_desktop.ps1")
    ...
    if ($desktopFailed) {
      ...
      Warn "  powershell -ExecutionPolicy Bypass -File src\scripts\install\install_desktop.ps1"
    }
  }
}

# --- 9. `aplyx` command on PATH ---------------------------------------------
$cliJs = Join-Path $projectRoot "src\tui\dist\cli.js"
