...
model: inherit
---
<!-- GENERATED from src/agents/bodies/job-scraper.md + src/agents/frontmatter/claude/job-scraper.yaml: edit those sources and run src/scripts/validate/generate_agent_definitions.py -->
...
- **No subagent registry** (no `@resume-tailor` / `@discord-reporter`
  available): read `src/agents/bodies/resume-tailor.md` or
  `src/agents/bodies/discord-reporter.md` at the delegation point and
...
3. For Workday tenants: use the deterministic fetch helper; only fall
   ...
   - Workday listings carry NO JD text. After role filtering (step 8)
    ...
    - **Workday is NOT review-only (phase 7D, 2026-08-28): Workday
      ...
      account-creation / verification / multi-step page-fill / final-
      submit flow end-to-end with the same fail-closed safety as the
...
3d. For Eightfold-powered careers sites (a white-labeled ATS many large
   ...
   strings, e.g. "apply.careers.microsoft.com/microsoft.com") and
   ...
   - Eightfold listings carry NO JD text in list mode, but (unlike an
     earlier, narrower version of this adapter) there IS a working
     per-posting detail endpoint. After role filtering (step 8) and
     BEFORE the fit gate (step 10), fetch the JD per surviving
     candidate:
     `python3 src/scripts/jobs/fetch_eightfold_listings.py --jd-url '<posting-url>'`
     then re-canonicalize and upsert with the fetched jd_text. Never
...
3e. For Apple (company-specific board, HTML-parsed, no public JSON
   ...
   - The list response carries only a truncated summary, not the full
     ...
     `python3 src/scripts/jobs/fetch_apple_listings.py --jd-url '<posting-url>'`
     then re-canonicalize and upsert with the fetched jd_text. Never
...
3h. For Gem-powered company boards: use the deterministic fetch
   ...
   - Gem listings carry NO JD text. After role filtering (step 8) and
     BEFORE the fit gate (step 10), fetch the JD per surviving
     candidate:
     `python3 src/scripts/jobs/fetch_gem_listings.py --jd-url '<posting-url>'`
     then re-canonicalize and upsert with the fetched jd_text. Never
...
8. Apply role filtering: a job title is a candidate if it contains at
   least one term from src/config/targets.json "role_keywords" AND at least
...
2W. **Workday only: do NOT drive the form via Playwright MCP yourself.**
   ...
   local runtime `src/scripts/runtime/approve_submit_workday.py`, which
   reuses the same checkpoint state format under
   `data/workday_apply_runs/<job_id>.json` and the same fail-closed
   ...
   a. **Select the account email.** Prefer a connected personal Gmail
      ...
      - Otherwise, if `src/config/targets.json "workday_alias_email"`
        ...
        compatibility path).
      - If neither is available, do NOT call the runtime and do NOT
        invent an email: route the job to needs_review with reasoning
        "Workday apply needs a connected personal Gmail inbox (use
        Continue Workday from the Review screen) or a managed alias
        email configured in src/config/targets.json
    ...
    b. Shell out to the runtime (do NOT pass `--no-submit`; it must
       submit when it reaches the confirmed final review/submit page):
        ...
        If rendering succeeds, pass `--resume-pdf logs/tmp/resume_<job_id>.pdf`;
        if it fails, route the job to needs_review under the normal missing-
       ...
       `--session-secret-file <path>` (a JSON file `{"link":...,"otp":...}`)
    ...
    c. Parse the runtime's JSON stdout. It always emits one JSON object
       ...
       `confirmation_url`, `filled_fields`, `resume_attached`,
      ...
      - `outcome == "submitted"` → status "applied" (the runtime
       ...
       - `outcome == "checkpoint"` and `checkpoint_status ==
         "awaiting_verification"` → keep the job pending in the Workday
         checkpoint and append an unresolved queue-only entry to
         `data/review_queue.json` with `status: "awaiting_verification"`,
         the canonical job fields, `apply_url`, and a specific reasoning
         message. Do NOT append `data/applied_jobs.json` or a terminal
         record-event yet. This queue row is an operational continuation
         handle, not a `needs_review` outcome; recording it in
         applied_jobs.json would make the mandatory can-apply recheck block
         the continuation. If the queue helper reports the job is already
         present, leave the existing row in place. This is the verification
         boundary: the scheduled path cannot cross it unattended because the
         local harness has no inbox/alias service to retrieve the Workday
         verification mail (see AGENTS.md "Inbox status detection
         (hosted-only, optional)"). The next run or the Review screen's
         Continue Workday action must supply the verification link or OTP;
         never pretend the account was verified, and never proceed to
         fill/submit on a page that is still the verification step. Do not
         run the terminal-outcome steps 8–11 for this temporary checkpoint.
       - `outcome == "checkpoint"` and `checkpoint_status ==
         "challenge_detected"` → status "needs_review", reasoning
         naming the challenge, doubt_signals `["captcha"]`.
       - `outcome == "checkpoint"` and `checkpoint_status ==
         "manual_required"` (TOTP/push/security-key/SSO/unsupported MFA
         the runtime cannot safely automate) → keep the job pending as
         an unresolved queue-only `awaiting_verification` entry (same
         shape as the awaiting_verification case above) with reasoning
         naming the `manual_required` label. Do NOT record applied or
         submitted; the user must complete that challenge manually and
         re-run. Never claim verified on ambiguity.
       - `outcome == "checkpoint"` and `checkpoint_status ==
         "ready_to_submit"` (page reached a submit button but was not
         clearly the final review/submit page) → status "needs_review",
        reasoning "Workday reached a submit button but the page was
        not clearly the final review/submit step; user to confirm the
        final submit manually via Continue Workday", doubt_signals
        `["submit_outcome_unclear"]`.
       - `outcome == "failed"` (validation_error / submit_click_failed
      ...
      - `ok == false` with no `outcome` (e.g. the runtime could not
        ...
        no apply_url) → status "needs_review", reasoning = the
        runtime's `message`.
      Carry `confirmation_url`, `filled_fields`, and
      `resume_attached` from the runtime into the applied_jobs.json /
      review_queue.json entry where present. The runtime owns the
      fill record (it writes `data/fill_records/` itself when fields
      are filled); do not call `record_fill.py` again from the agent
      for a Workday job, and set `fill_record_path` only if the
      runtime returned one.
   d. Resume on a later run: when the same Workday job is re-processed
      (by the scheduler, or by the user-driven Continue Workday action
      in the TUI which calls the same runtime with a supplied
      `--verification-link`/`--otp`), the runtime loads its existing
      checkpoint and continues from `awaiting_verification` → login →
...
3. Fill form fields using src/config/targets.json "safe_fields" only.
   ...
   **Dropdowns, comboboxes, and typeaheads: never accept an unconfirmed
   ...
   d. **Still no exact match, or several options match equally?** Do NOT
      ...
      5 options seen>; user to apply manually"`, doubt_signals including
      `"ambiguous_dropdown"` (see AGENTS.md "Doubt signals"), and move to
      the next job. A wrong city on a submitted application cannot be
...
6a. **Persist the fill record.** Immediately after step 6, regardless of
   ...
   Take the `path` from its JSON output and carry it forward as
...
- ALWAYS run the deterministic JD fit gate
  (src/scripts/jobs/evaluate_job_fit.py) on every canonical job after role
...
- CAPTCHA detected → stop applying to that board, log all pending jobs as
  "needs_review" in applied_jobs.json and via record-event (doubt_signals:
  `"captcha"`), notify Discord via @discord-reporter (needs_review route)
  for each, continue with other boards.
- Form field not recognized → do not guess and do not skip it silently. If
  the field is optional, leave it blank and continue the form. If it's
  required (or you can't tell), stop and route the job to needs_review with
  doubt_signals including `"unrecognized_field"` (see Phase 3 step 3) rather
  than submitting a form with an unknown required field left empty.
