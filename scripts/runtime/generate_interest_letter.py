...
prompt from agents/bodies/interest-letter.md's body verbatim, calls
...
the model's own grounding rules (agents/bodies/interest-letter.md) — it
...
def read_anthropic_key(root: str) -> str | None:
    ...
    config/anthropic_key.json (gitignored local-file fallback, same
    pattern as config/job_cache_redis.json) — never a committed default,
    this key is billed per-token and must never ship in any bundle."""
    env_key = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if env_key:
        return env_key
    parsed = _read_json(os.path.join(root, "config", "anthropic_key.json"), {})
...
def _read_agent_body(root: str) -> str:
    path = os.path.join(root, "agents", "bodies", "interest-letter.md")
    with open(path, "r", encoding="utf-8") as fh:
        return fh.read()
...
def _grounding_flags(letter: str, company: str, reported_word_count: int) -> list[str]:
    ...
    grounding rules (agents/bodies/interest-letter.md) weren't followed,
    ...
    # name this operator has ever targeted (config/targets.json's own
...
def main(argv=None) -> int:
    ...
    if not api_key:
        print(json.dumps({"ok": False, "error": "no Anthropic API key configured "
                                                  "(set ANTHROPIC_API_KEY or config/anthropic_key.json)"}))
        return 2
    ...
    try:
        ...
        print(json.dumps({"ok": False, "error": f"could not read agents/bodies/interest-letter.md: {exc}"}))
        return 2
