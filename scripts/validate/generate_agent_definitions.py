...
Single source of truth for agent behavior:
  ...
  agents/frontmatter/copilot/<name>.yaml   Copilot CLI frontmatter
  agents/frontmatter/codex/<name>.toml     Codex CLI metadata (TOML, no body)
...
TOML_MARKER = (
    "# GENERATED from agents/bodies/{name}.md + "
    "agents/frontmatter/{harness}/{name}.toml — edit those sources and run "
    "scripts/validate/generate_agent_definitions.py"
)
...
def compose_toml(root: str, harness: str, name: str) -> str:
    body_path = os.path.join(root, "agents", "bodies", f"{name}.md")
    fm_path = os.path.join(root, "agents", "frontmatter", harness, f"{name}.toml")
    with open(fm_path, "r", encoding="utf-8") as f:
    ...
    if "'''" in body:
        raise ValueError(
            f"agents/bodies/{name}.md contains a literal ''' sequence, which "
...
def source_path(root: str, harness: str, name: str) -> str:
    return os.path.join(root, "agents", "frontmatter", harness, f"{name}.{SRC_EXT[harness]}")
