...
Mirrors packages/core/src/jobCache.ts / jobCacheRedis.ts / jobs.ts's
...
documents on the TS side:
  ...
  1. Upstash Redis (config/job_cache_redis.json's read-only token) —
  ...
  2. The Postgres job_cache_search RPC (config/job_cache_supabase.json's
...
committed list in config/job_cache_targets.json is always cache-checked;
whichever of a source's *personal* config/targets.json companies aren't
...
DEFAULT_JOB_CACHE_TARGETS = "config/job_cache_targets.json"
DEFAULT_REDIS_CONFIG = "config/job_cache_redis.json"
DEFAULT_SUPABASE_CONFIG = "config/job_cache_supabase.json"
...
def shared_slugs(root: str, source: str) -> set[str]:
    ...
    config/targets.json. Never raises; a missing/unreadable file just
